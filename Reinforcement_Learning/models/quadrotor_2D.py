...
from tf_rl.simulation.quadrotor import Quadrotor
...
brain = MLP([6, ], [32, 64,  N*N], [tf.tanh, tf.tanh, tf.identity])

optimizer = tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.9)

current_controller = DiscreteDeepQ(6, N*N, brain, optimizer, session,
                                   discount_rate=0.9, exploration_period=500000,
...
dt = 0.02
try:
    for game_idx in range(20000):
        game = Quadrotor(QUADROTOR_PARAMS)
        game_iterations = 0
        ...
        rewards = []
        cost0 = game.cost()
        while game_iterations < 200 and not game.is_over():
        ...
        if game_idx % 100 == 0:
            ...
            save_path = saver.save(current_controller.s,
                                   "./saved_models/quadrotor/model_r"+str(game_idx)+".ckpt")
