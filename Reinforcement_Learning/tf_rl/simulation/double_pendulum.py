...
class DoublePendulum(object):
    ...
    def __init__(self, params):
        ...
        max_control_input : float
            ...
            to the first joint
        """
        x = 2*np.pi*np.random.rand(2)-np.pi
        v = np.array([0, 0])
        self.state = np.append(x,v)
        self.control_input = 0.0
        self.params = params
        self.size = (400, 300)
        self.actions = np.linspace(-1, 1, 201)
    ...
    def perform_action(self, action):
        ...
        self.control_input = self.actions[action] * self.params['max_control_input']
    ...
    def is_over(self):
        ...
        L2 = self.params['l2_m']
        _, joint2 = self.joint_positions()
        return np.fabs(joint2[1]-L1-L2) < 1e-3 and\
            np.fabs(self.state[1])+np.fabs(self.state[3]) < 5e-2
